...
local function generateCode(graph, opt)
   ...
   if useTerra then
     out:writeln("local gradFun = function(", table.concat(paramSymbols, ", "), ")")
   else
   ...
   out:writeln("end")
   if useTerra then
     out:writeln("return compileTrace(gradFun,rlocals)")
   end
   out:writeln("end")
...
local function generateFn(graph, opt)
   ...
   --for i,v in ipairs(outerArgs) do
   --  print(v)
   --end
